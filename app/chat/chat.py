...
class Chat:
    ...
    @classmethod
    async def from_db(cls, chat_id: str, prompt: Optional[str] = None):
        chat: Optional[dict[str, Any]] = await chat_collection.find_one({"_id": chat_id})
        if chat is None:
            if prompt is None:
        ...
        sanity_counter = await ChatCounter.from_db(chat_id, "sanity_counter")
        messages: ChatList[dict[str, Any]] = await ChatList.from_db(chat_id, "messages")
        if len(messages) == 0:
        ...
        function_names: ChatList[str] = await ChatList.from_db(chat_id, "function_names")
        if len(function_names) == 0:
            predicted_function_names = await predict_functions(_prompt)
            await function_names.extend(predicted_function_names)

        return cls(chat_id, _prompt, messages, function_names, sanity_counter)

    async def functions(self):
        system_functions = await load_functions_by_namespace("system")
        functions = await load_functions(list(self._function_names))
